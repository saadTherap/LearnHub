...
import static net.therap.server.app.enums.ExceptionTypes.*;
...
public class AuthServiceImpl implements AuthService {
    ...
    public JwtResponse login(LoginRequest request) {
        ...
        if (!user.isEnabled()) {
            throw new AuthServerException(messageUtil.getMessage("err.user.not_enabled"), LOGIN_ERROR_EXCEPTION);
        }
    ...
    public JwtResponse refreshToken(String refreshToken) {
        ...
        if (!jwtService.isValid(refreshToken)) {
            throw new AuthServerException(messageUtil.getMessage("err.refresh.token.invalid"), REFRESH_TOKEN_ERROR_EXCEPTION);
        }

        User user = getUser(email);

        if (!user.isEnabled()) {
            throw new AuthServerException(messageUtil.getMessage("err.user.not_enabled"), REFRESH_TOKEN_ERROR_EXCEPTION);
        }
    ...
    public void verifyEmail(String token) {
        VerificationToken verificationToken = verificationTokenRepository.findByToken(token)
                .orElseThrow(() -> new AuthServerException(
                        messageUtil.getMessage("err.verify.token.invalid"), MAIL_VERIFICATION_ERROR_EXCEPTION)
                );
        ...
        if (verificationToken.isExpired()) {
            ...
            throw new AuthServerException(
                    messageUtil.getMessage("err.verify.token.expired"), MAIL_VERIFICATION_ERROR_EXCEPTION);
        }
    ...
    private User authenticateUser(String email, String password) {
        ...
        if (Objects.isNull(user)) {
            throw new AuthServerException(messageUtil.getMessage("err.auth.invalid_credentials"), LOGIN_ERROR_EXCEPTION);
        }
